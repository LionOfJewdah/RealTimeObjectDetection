...
{
	size_t number = layer.c * layer.batch;

	forward_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK>>>(
		number, layer.w, layer.h, layer.c, net.input_gpu, layer.output_gpu);
...
extern "C" void backward_avgpool_layer_gpu(avgpool_layer layer, network net)
{
	...
	backward_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK>>>(
		number, layer.w, layer.h, layer.c, net.delta_gpu, layer.delta_gpu);
	check_error(cudaPeekAtLastError());
}
