...
detection_layer make_detection_layer(int batch, int inputs, int n,
	int side, int classes, int coords, int rescore)
{
	detection_layer l = { 0 };
	l.type = DETECTION;
...
void forward_detection_layer(const detection_layer l, network net)
{
	...
	if (net.train) {
		...
		*(l.cost) = 0;
		...
		for (b = 0; b < l.batch; ++b) {
			...
			for (i = 0; i < locations; ++i) {
				...
				box truth = float_to_box(net.truth + truth_index +
					1 + l.classes, 1);
				truth.x /= l.side;
				truth.y /= l.side;
				...
				int p_index = index + locations *
					l.classes + i * l.n + best_index;
				*(l.cost) -= l.noobject_scale * pow(l.output[p_index], 2);
				*(l.cost) += l.object_scale * pow(1 - l.output[p_index], 2);
				...
				l.delta[box_index + 0] =
					l.coord_scale * (net.truth[tbox_index + 0] -
						l.output[box_index + 0]);
				l.delta[box_index + 1] =
					l.coord_scale * (net.truth[tbox_index + 1] -
						l.output[box_index + 1]);
				l.delta[box_index + 2] =
					l.coord_scale * (net.truth[tbox_index + 2] -
						l.output[box_index + 2]);
				l.delta[box_index + 3] =
					l.coord_scale * (net.truth[tbox_index + 3] -
		...
		if (0) {
			...
			for (b = 0; b < l.batch; ++b) {
				...
				for (i = 0; i < locations; ++i) {
					for (j = 0; j < l.n; ++j) {
						int p_index =
							index + locations * l.classes + i * l.n + j;
		...
		*(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
		...
		printf("Detection Avg IOU: %f, Pos Cat: %f, All Cat: %f, Pos Obj: %f,"
			"Any Obj: %f, count: %d\n",
