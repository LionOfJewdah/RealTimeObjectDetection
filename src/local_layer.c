                             ...
                             int stride, int pad, ACTIVATION activation)
{
	...
	l.weights_gpu        = cuda_make_array(l.weights,
...
void backward_local_layer(local_layer l, network net)
{
	...
	for (i = 0; i < l.batch; ++i) {
		...
		if (net.delta) {
			...
			col2im_cpu(net.workspace, l.c, l.h, l.w, l.size, l.stride,
			           l.pad, net.delta + i * l.c * l.h * l.w);
		}
	}
}
...
void backward_local_layer_gpu(local_layer l, network net)
{
	...
	for (i = 0; i < l.batch; ++i) {
		...
		if (net.delta_gpu) {
			...
			col2im_gpu(net.workspace, l.c, l.h, l.w, l.size, l.stride,
			           l.pad, net.delta_gpu + i * l.c * l.h * l.w);
		}
	}
}
