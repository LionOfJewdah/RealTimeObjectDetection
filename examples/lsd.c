...
void inter_dcgan(char *cfgfile, char *weightfile) {
	...
	while (1) {
		...
		slerp(start.data, end.data, (float)count / max_count, im.w * im.h * im.c,
					im.data);
	...
	while (get_current_batch(gnet) < maxbatch) {
		...
		for (j = 0; j < gnet->subdivisions; ++j) {
			...
			*gnet->seen += gnet->batch;
			...
			scal_gpu(imlayer.outputs * imlayer.batch, 0,
							 gnet->layers[gnet->n - 1].delta_gpu, 1);

			axpy_gpu(imlayer.outputs * imlayer.batch, 1, imerror, 1,
							 gnet->layers[gnet->n - 1].delta_gpu, 1);
		...
		printf("%d: adv: %f | adv_avg: %f, %f rate, %lf seconds, %d images\n", i,
					 aloss, aloss_avg, get_current_rate(gnet), sec(clock() - time),
					 i * imgs);
		if (i % 10000 == 0) {
			char buff[256];
	...
	while (get_current_batch(net) < net->max_batches) {
		...
		for (j = 0; j < net->subdivisions; ++j) {
			...
			*net->seen += net->batch;
			...
			fill_gpu(imlayer.outputs * imlayer.batch, 0, imerror, 1);
			copy_gpu(anet->inputs * anet->batch, imlayer.output_gpu, 1,
							 anet->input_gpu, 1);
			...
			scal_gpu(imlayer.outputs * imlayer.batch, 1. / 100.,
			...
			for (k = 0; k < net->batch; ++k) {
				...
				copy_cpu(imlayer.outputs, imlayer.output + k * imlayer.outputs, 1,
								 gray.X.vals[index], 1);
			}
		}
		harmless_update_network_gpu(anet);
		...
		printf("%d: gen: %f, adv: %f | gen_avg: %f, adv_avg: %f, %f rate, %lf "
