...
float_pair get_rnn_vid_data(network net, char** files, int n, int batch,
	int steps)
{
	int b;
	...
	for (b = 0; b < batch; ++b) {
		...
		for (i = 0; i < net.batch; ++i) {
			memcpy(feats + (b + i * batch) * output_size,
				output + i * output_size, output_size * sizeof(float));
		}
...
image save_reconstruction(network net, image* init, float* feat, char* name,
	int i)
{
	image recon;
	if (init) {
...
void generate_vid_rnn(char* cfgfile, char* weightfile)
{
	...
	int i;
	CvCapture* cap = cvCaptureFromFile("/extra/vid/ILSVRC2015/Data/"
		"VID/snippets/val/ILSVRC2015_val_00007030.mp4");
	float* feat;
	float* next;
	image last;
	for (i = 0; i < 25; ++i) {
		...
		if (i > 0) {
			printf("%f %f\n", mean_array(feat, 14 * 14 * 512),
				variance_array(feat, 14 * 14 * 512));
			printf("%f %f\n", mean_array(next, 14 * 14 * 512),
				variance_array(next, 14 * 14 * 512));
